                        ...
                        match delete_index_regex.captures(string){
                            ...
                            None =>{
                                ...
                                match array_slice_regex.captures(string){
                                    Some(needle) =>{
                                        output.push((Commands::Filter(FilterType::ArraySlice),String::from(&needle[1])))
                                    }
                                    None =>{
                                        //check if it is an array index
                                        match array_index_regex.captures(string){
                                            Some(needle) => output.push((Commands::Filter(FilterType::ArrayIndex),String::from(&needle[1]))),
                                            None =>{
                                                ...
                                                output.push((Commands::Filter(FilterType::ObjectIdentity),String::from(&needle[1])))
                                            }
                                        }
                                    }
                                }   
...
mod test{
    ...
    fn test_filter_identity(){
        ...
        assert_eq!(filterneedle(input),vec![(Commands::Filter(FilterType::Identity),String::new())])
    }
    #[test]
    fn test_filter_delete(){
        let input = String::from("del(.test)");
        assert_eq!(filterneedle(input),vec![(Commands::Delete,String::from("test"))]);
        let input = String::from("del[.0]");
        assert_eq!(filterneedle(input),vec![(Commands::Delete,String::from("0"))]);
    }
    #[test]
    fn test_filter_array_index(){
        //testing array index
        let input = String::from(".[0]");
        assert_eq!(filterneedle(input),vec![(Commands::Filter(FilterType::ArrayIndex),String::from("0"))]);
        //even though this is an invalid input we want to catch it later with our filter code
        let input = String::from(".[balal]");
        assert_eq!(filterneedle(input),vec![(Commands::Filter(FilterType::ArrayIndex),String::from("balal"))]);
    }
    #[test]
    fn test_filter_array_slice(){
        //testing array slice
        let input = String::from(".[0:10]");
        assert_eq!(filterneedle(input),vec![(Commands::Filter(FilterType::ArraySlice),String::from("0:10"))]);
        //even though this is an invalid input we want to catch it later with our filter code
        let input = String::from(".[balal:rand]");
        assert_eq!(filterneedle(input),vec![(Commands::Filter(FilterType::ArraySlice),String::from("balal:rand"))]);
    }
    #[test]
    fn test_filter_object_index(){
        //testing array index
        let input = String::from(".filter");
        assert_eq!(filterneedle(input),vec![(Commands::Filter(FilterType::ObjectIdentity),String::from("filter"))]);
    }
    #[test]
    fn test_filter_array_iterator(){
        //testing array index
        let input = String::from(".[]");
        assert_eq!(filterneedle(input),vec![(Commands::ArrayIterator,String::new())]);
    }
    //Pipe tests
    #[test]
    fn test_filter_pipe(){
        ...
        let input = String::from(".|add");
        let output = vec![(Commands::Filter(FilterType::Identity),String::new()),(Commands::Add,String::new())];
        assert_eq!(filterneedle(input),output);
        //try something more complicated
        let input = String::from(".[]|.name");
        let output = vec![(Commands::ArrayIterator,String::new()),(Commands::Filter(FilterType::ObjectIdentity),String::from("name"))];
        assert_eq!(filterneedle(input),output);
    }
